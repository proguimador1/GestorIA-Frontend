...
export default function RedefinirSenha() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setMensagem('O link é inválido ou já expirou. Solicite um novo link.');
        }
    };
        ...
        <div style={{ maxWidth: '400px', margin: '50px auto', fontFamily: 'sans-serif' }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                ...
                <input
                    ...
                    style={{ padding: '8px' }}
                />
                <button type="submit" disabled={status === 'loading' || status === 'success'} style={{ padding: '10px' }}>
                    {status === 'loading' ? 'Salvando...' : 'Salvar Nova Senha'}
                </button>
            </form>
            {mensagem && <p style={{ marginTop: '15px', color: status === 'error' ? 'red' : 'green' }}>{mensagem}</p>}
        </div>
    );
}